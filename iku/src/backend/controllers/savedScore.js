...
import { getSavedScoresByLocations, getOverallSavedScoresByLocation, createSavedScore, removeSavedScore } from "../models/savedScoreModel.js";
...
export const showOverallSavedScoresByLocation = (req, res) => {
    getOverallSavedScoresByLocation(req.params.origin, (err, results) => {
        if (err){
            res.send(err);
        }else{
            res.json(results);
